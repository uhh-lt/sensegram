...
class SenseClusterDisambiguator(object):
    ...
    def run(self, output_fpath, normalize=True, output_sim=False, skip_ambigous=False):
        ...
        with codecs.open(output_fpath, "w", "utf-8") as output_file:
            print >> output_file, "word\tcid\tcluster\tisas"
            err_num = 0
            i = 0
            ...
            for i, word in enumerate(self._sc.keys()):
                ...
                for sense_id in self._sc[word]:
                    ...
                    ddt_isas = self._disambiguate(target_field="isas", context_fields=["cluster","isas"], sense_fields=["cluster"],
                        normalize=normalize, sense_id=sense_id, word=word, skip_ambigous=skip_ambigous)
                    print >> output_file, "%s\t%s\t%s\t%s" % (word, sense_id,
                        self._format_cluster(ddt_cluster, output_sim), self._format_cluster(ddt_isas, output_sim))
    ...
    def _format_cluster(self, ddt_cluster, output_sim):
        if output_sim:
            return ','.join(map(lambda x: "%s%s%.3f" % (x[0], SEP3, float(x[1])),
                sorted(ddt_cluster.items(), key=itemgetter(1), reverse=True)))
        else:
            return ','.join(map(lambda x: x[0], sorted(ddt_cluster.items(), key=itemgetter(1), reverse=True)))
            ...
            try:
                ...
                if cword in self._sc:
                    ...
                    if len(cword_invoc) == 0 and not skip_ambigous:
                        disambiguated_field[cword + SEP4 + u"?"] = self._sc[word][sense_id][target_field][cword]
                        continue

                # if present then select the best sense
                senses = [(u"?", context, cword)]
                ...
                if scores[best_i][0] > MIN_SCORE:
                    disambiguated_field[senses[best_i][2] + SEP4 + unicode(senses[best_i][0])] = self._sc[word][sense_id][target_field][cword]
                else:
                    if not skip_ambigous: disambiguated_field[cword + SEP4 + u"?"] = self._sc[word][sense_id][target_field][cword]
