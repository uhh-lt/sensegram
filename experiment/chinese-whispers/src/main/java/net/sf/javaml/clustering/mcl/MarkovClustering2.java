...
public class MarkovClustering2 {
     ...
     */
    public Matrix run(Matrix a, double maxResidual, double pGamma, double loopGain, double maxZero) {
        // add cycles
        addLoops(a, loopGain);
        a.normaliseRows();
        double residual = 1.;
     ...
     */
    public double inflate(Matrix m, double p, double zeromax) {
        double res = 0.;
     ...
     * @param loopGain
     */
    private void addLoops(Matrix a, double loopGain) {
        if (loopGain <= 0) {
            return;
        }
